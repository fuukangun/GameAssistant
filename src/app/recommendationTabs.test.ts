...
test('creates dashboard tabs in display order', () => {
  ...
  assert.deepEqual(tabs.map((tab) => tab.id), ['reminders', 'actions', 'skills', 'friendship', 'exploration', 'inventory']);
  assert.equal(tabs[0].label, '重要提醒');
  assert.equal(tabs[1].label, '推荐行动');
  assert.equal(tabs[2].label, '角色技能');
  assert.equal(tabs[3].label, '友谊维系');
  assert.equal(tabs[4].label, '探索进度');
  assert.equal(tabs[5].label, '拥有物品');
});
